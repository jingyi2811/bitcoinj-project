...
//            Futures.addCallback(balanceFuture, callback);
            ...
            CompletableFuture<Coin> balanceCompletableFuture = kit.wallet().getBalanceCompletableFuture(value, BalanceType.AVAILABLE);
            
            balanceCompletableFuture.exceptionally(error -> {
            	System.out.println("something went wrong");
                return null;
            });
           
            balanceCompletableFuture.thenAccept(success -> {
            	System.out.println("coins arrived and the wallet now has enough balance");
            });
        }
